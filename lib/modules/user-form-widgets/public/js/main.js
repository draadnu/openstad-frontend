apos.define('user-form-widgets', {
    ...
    construct: function (self, options) {
        self.play = function ($widget, data, options) {
            ...
            if ($widget.find('.anders_checkbox').length > 0) {
              $widget.find('.anders_checkbox input.anders').each(function () {
                $(this).on('change', function () {
                  if ($(this).is(':checked')) {
                    $(this).closest('.anders_checkbox').next('.anders_input').removeClass('hidden');
                  } else {
                    $(this).closest('.anders_checkbox').next('.anders_input').addClass('hidden').find('input').val('');
                  }
                }).trigger('change');
              })
            }
