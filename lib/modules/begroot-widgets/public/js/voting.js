...
if (votingContainer !== null) {
  ...
  var currentPlaygrounds = openstadGetStorage('currentPlaygrounds') || [];
  ...
  function toggleIdeaInSelection(id) {
	  ...
	  var playgroundIndex = currentPlaygrounds.indexOf(element.playground);
	  
	  console.log ('playgroundIndex:', playgroundIndex);
	  
	  if (index == -1 && playgroundIndex == -1) {
  ...
  function addIdeaToSelection(id) {
	  ...
	  console.log (currentPlaygrounds);
	  ...
	  currentPlaygrounds.push(element.playground);

	  recalculateAvailableAmount();
	  
	  console.log (element.playground);
    ...
    var storeSelection = votingType == 'budgeting-per-theme' ? themes.reduce( function(result, theme) { return result.concat( theme.currentSelection ) }, []) : currentSelection;
	  ...
	  openstadSetStorage('currentPlaygrounds', currentPlaygrounds)
  ...
  function removeIdeaFromSelection(id) {
	  ...
	  currentPlaygrounds.splice(playgroundIndex, 1);
  ...
  function initSortedElements() {
	  ...
	  for (var i=0; i<elements.length; i++) {
		  ...
		  element.playground = $('li#idea-' + element.ideaId).data('playground');
			    ...
			    if (
				    (votingType === 'count' && maxIdeas <= currentSelection.length)
				      || element.budgetValue > availableBudgetAmount
							|| currentPlaygrounds.indexOf( element.playground ) != -1
			    ) {
