...
const fields = [
  ...
  {
    name: 'showOnlyAcceptedIdeas',
    type: 'boolean',
    label: 'Only show accepted ideas',
    def: false
  },
  {
    type: 'checkboxes',
  ...
  construct: function(self, options) {
    ...
    /*    const acceptedIdeas = req.data.ideas ? req.data.ideas.filter(idea => idea.status === 'ACCEPTED') : []; */
        widgets.forEach((widget) => {
            ...
            if (widget.showOnlyAcceptedIdeas) {
                widget.acceptedIdeas = widget.acceptedIdeas ? widget.acceptedIdeas.filter(idea => idea.status === 'ACCEPTED') : [];
            }
        });
