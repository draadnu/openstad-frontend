...
module.exports = {
    ...
    addFields: [
      ...
      {
        type: 'boolean',
        name: 'disableRightSideLink',
        label: 'Disable right side link',
        choices: [
          {
            value: true,
            label: "Yes",
        ...
        {
            name: '_ideaOverviewPage',
            withType: 'apostrophe-page',
            type: 'joinByOne',
            idField: 'ideaOverviewPageId',
            label: 'Link to idea overview page',
            filters: {
                projection: {
                    _url: 1,
                    title: 1
        ...
        {
            name: 'ideaOverviewButtonText',
            type: 'string',
            label: 'Text on the plan overview button',
        },
        {
            type: 'boolean',
            name: 'hideRightSideColumn',
            label: 'Hide right side column',
            choices: [
                {
                    value: true,
                    label: "Yes",
        ...
        self.load = function (req, widgets, callback) {
            widgets.forEach((widget) => {
                ...
                if (widget.hasFixedIdeaCount) {
                  ...
                  if (widget.argumentCount) {
                    widget.ideaCountParts = req.data.ideas ? _.reduce(req.data.ideas, (count, idea) => { return count + (idea.argCount ? idea.argCount : 0)}, 0).toString().split("") : false;
                  } else {
                ...
                if (req.query.ideaId) {
                    widget.btn1Url = widget.btn1Url.replace(':ideaId', req.query.ideaId);
                    widget.btn2Url = widget.btn2Url.replace(':ideaId', req.query.ideaId);
                }
