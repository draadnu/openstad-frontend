  ...
  construct: function(self, options) {
    ...
    self.load = (req, widgets, callback) => {
        ...
        widgets.forEach((widget) => {
            req.data.ideas = self.getIdeas(widget, req.data.ideas)
    ...
    self.getIdeas = (widget, ideas) => {
      if ((widget.showIdeasByStatus|| []).length > 0 && ideas && ideas.length > 0) {
        return ideas.filter(idea => widget.showIdeasByStatus.some(status => status === idea.status));
      }
