...
const getVotes = async (ideaId, req, apiUrl) => {
    const siteId = req.data.global.siteId;
    const url = `${apiUrl}/api/site/${siteId}/vote?ideaId=${ideaId}`;
  ...
  addFields: [
      {
          name: 'mapType',
          type: 'select',
          label: 'What kind of map do you want to use?',
          choices: [
              {
                  value: "openlayers",
                  label: "Openlayers"
  ...
  construct: function(self, options) {
     ...
     self.pageBeforeSend = (req, callback) => {
        ...
        if (ideaId) {
            ...
            getVotes(ideaId, req, apiUrl).then(function (response) {
                req.data.idea.votes = response;
            }).catch(function (err) {
