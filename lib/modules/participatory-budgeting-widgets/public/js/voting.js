...
if (votingContainer !== null) {
  ...
  function saveExtraVotesInStorage () {
  	...
  	console.log ('saving extra votes', extraVotes);
            ...
            overviewHtml = '<h5 class="overview-title">' + title + '</h5><div class="overview"><table cellpadding="0" class="table-center stretch">' + overviewHtml + '';
			    ...
			    function createInformationElement (ideaElement) {
			    	if (allowExtraVotes && extraVoteCount > 0) {
			    		var ideaTitle = ideaElement.querySelector('.intro').cloneNode(true).innerText;

							var el = document.createElement('span');
							el.setAttribute('title', ideaTitle.trim());
							el.className = 'information-circle';
  ...
  function submitBudget() {
    ...
    console.log (allowExtraVotes, extraVoteCount, openstadGetStorage('extraVotes'));
  ...
  function handleClick(event) {
    ...
    if (ideaElement) {
      ...
      var ideaMap = _.filter(ideaMaps, function (idea) {
        return idea.id === ideaId;
      });

      if (ideaMap.length <= 0) {
        // Fixme: hacky timeout for gridder-show
        setTimeout(function () {
          var nlmapsHolder = $('.gridder-show #nlmaps-idea-holder-' + ideaId);
          if (nlmapsHolder.length > 0) {
