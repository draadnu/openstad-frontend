...
if (votingContainer !== null) {
  ...
  var allowExtraVotes = allowExtraVotes || false;
  var extraVoteCount = extraVoteCount || 0;
  ...
  function previousStep() {
	  ...
	  if (currentStep == 1 && allowExtraVotes && extraVoteCount) {
			saveExtraVotesInStorage();
			addToClassName(document.querySelector('div.extra-votes-passed'), 'hide');
		}
  ...
  function nextStep() {
	  ...
	  if (currentStep == 2 && allowExtraVotes && extraVoteCount) {
	  	saveExtraVotesInStorage();
			addToClassName(document.querySelector('div.extra-votes-passed'), 'hide');
		}
  ...
  function saveExtraVotesInStorage () {
  	var extraVotes = [];

  	document.querySelectorAll('input.extra-votes').forEach(function (el) {
  		var ideaId = el.getAttribute('data-idea-id'),
					votes = el.value;
  	...
  	openstadSetStorage('extraVotes', extraVotes);
	}

	function loadExtraVotesFromStorage () {
  	var extraVotes = openstadGetStorage('extraVotes');

  	if (extraVotes) {
  		extraVotes.forEach(function (extraVote) {
  			var input = document.querySelector('#extra-vote-' + extraVote.ideaId);

  			if (input) {
  				input.value = extraVote.votes;
				}
			})
		}
	}
	
	function countExtraVotes () {
  	var voteCount = 0;
          ...
          function createOverview(selection, initialAvailableBudget, availableBudgetAmount, title) {
			      ...
			      selection.forEach(function(id) {
				      ...
				      if (allowExtraVotes) {
				      	...
				      	var infoEl = createInformationElement(element);
								overviewHtml += '<td class="text-align-right information-column">' + infoEl + '</td>';

								var extraVoteEl = createExtraVoteElement(id);
				      	overviewHtml += '<td class="text-align-right extra-vote-column">' + extraVoteEl + '</td>';
							}
            ...
            overviewHtml = '<h5 class="overview-title">' + title + '</h5><div class="overview"><table cellpadding="0" class="table-center stretch">' + overviewHtml + '';
			      ...
			      if (allowExtraVotes && extraVoteCount) {
			      	document.querySelectorAll('a.extra-vote').forEach(function (el) {
			      		el.addEventListener('click', addOrSubtractExtraVote);
							});

							document.querySelectorAll('input.extra-votes').forEach(function (el) {
								el.addEventListener('change', validateExtraVotes);
							});

							// Load correct vote counts from storage upon load
							loadExtraVotesFromStorage();

							// Run validate extra votes upon load
							validateExtraVotes();
						}
          }

			    function createInformationElement (ideaElement) {
			    	if (allowExtraVotes && extraVoteCount > 0) {
			    		var ideaSummary = ideaElement.querySelector('.intro').cloneNode(true).innerText;
			    		
							var el = document.createElement('span');
							el.setAttribute('aria-label', ideaSummary.trim());
							el.setAttribute('data-toggle', 'tooltip');
							el.className = 'information-circle hint--top hint--large';
							
							return el.outerHTML;
						}
					}

			    function createExtraVoteElement (ideaId) {
			    	if (allowExtraVotes && extraVoteCount > 0) {
			    		var el = document.createElement('div'),
									extraVoteInputElementId = 'extra-vote-' + ideaId;

			    		var voteInput = document.createElement('input');
			    		voteInput.setAttribute('type', 'number');
			    		voteInput.setAttribute('data-idea-id', ideaId);
			    		voteInput.setAttribute('id', extraVoteInputElementId);
			    		voteInput.setAttribute('min', '1');
			    		voteInput.setAttribute('max', (extraVoteCount + 1).toString());
			    		voteInput.setAttribute('value', getExtraVoteValue(ideaId));
			    		voteInput.className = 'extra-votes';

			    		var decreaseButton = document.createElement('a');
			    		decreaseButton.setAttribute('data-idea-id', ideaId);

			    		var span = document.createElement('span');
			    		span.textContent = 'Verlaag aantal stemmen';
			    		span.className = 'sr-only';

			    		decreaseButton.appendChild(span);

			    		var increaseButton = decreaseButton.cloneNode();

			    		var secondSpan = span.cloneNode();
			    		secondSpan.textContent = 'Verhoog aantal stemmen';
			    		increaseButton.appendChild(secondSpan);

			    		decreaseButton.className = 'extra-vote decrease';

							increaseButton.className = 'extra-vote increase';

							var label = document.createElement('label');
							label.setAttribute('for', extraVoteInputElementId);
							label.textContent = 'Aantal stemmen';
							label.className = 'sr-only';

							el.appendChild(decreaseButton);
			    		el.appendChild(label);
			    		el.appendChild(voteInput);
			    ...
			    function getExtraVoteValue (ideaId) {
			    	return 1;
					}

			    function addOrSubtractExtraVote (event) {
			    	...
			    	var input = button.parentNode.querySelector('input[data-idea-id]');

			    	if (button.className.match('decrease')) {
			    		input.value = Math.max(1, parseInt(input.value) - 1)
						} else if (button.className.match('increase')) {
			    		input.value = Math.min((extraVoteCount + 1), parseInt(input.value) + 1)
						}

			    	validateExtraVotes(input);
					}

			    function validateExtraVotes (input) {
			    	var maxVotes = currentSelection.length + extraVoteCount;

			    	totalVotes = 0;

			    	document.querySelectorAll('input.extra-votes').forEach(function (el) {
							totalVotes += parseInt(el.value);
						});

			    	// Reset all extra votes if we have too many votes
			    	...
			    	document.querySelectorAll('input.extra-votes').forEach(function (el) {
			    		if (parseInt(el.value) === 1) {
			    			el.parentNode.querySelector('a.extra-vote.decrease').setAttribute('disabled', 'disabled');
							} else {
			    			el.parentNode.querySelector('a.extra-vote.decrease').removeAttribute('disabled');
							}

			    		if (parseInt(el.value) === (extraVoteCount + 1) || totalVotes === maxVotes) {
			    			el.parentNode.querySelector('a.extra-vote.increase').setAttribute('disabled', 'disabled');
							} else {
			    			el.parentNode.querySelector('a.extra-vote.increase').removeAttribute('disabled');
							}
						});

			    	document.querySelector('.current-extra-votes-count').textContent = (totalVotes - currentSelection.length);
			    	document.querySelector('.available-extra-votes-count').textContent = (maxVotes - totalVotes);

			    	var extraVotesPassedElement = document.querySelector('div.extra-votes-passed');

			    	if (totalVotes === maxVotes) {
			    		removeFromClassName(extraVotesPassedElement, 'hide');
						} else {
			   			addToClassName(extraVotesPassedElement, 'hide');
						}
  ...
  function isSelectionValid() {
    ...
    switch (votingType) {
      case 'count':
      	if (allowExtraVotes && currentStep === 2) {
      			return currentSelection.length >= minIdeas && currentSelection.length <= maxIdeas && countExtraVotes() > 0 && countExtraVotes() <= extraVoteCount;
				}
    ...
    if (votingType == 'budgeting-per-theme') {
	  ...
	  if (allowExtraVotes && extraVoteCount) {
			...
			extraVotes.forEach(function (extraVote) {
				...
				for (var i = 0; i < extraVote.votes; i++) {
					votesToSubmit.push({
						opinion: "yes",
						ideaId: extraVote.ideaId
					})
				}
			});
      ...
      success: function(data) {
			  ...
			  openstadRemoveStorage('extraVotes');
			  availableBudgetAmount = initialAvailableBudget;
			  //nextStep();
  ...
  function initSortedElements() {
	  ...
	  for (var i=0; i<elements.length; i++) {
		  ...
		  element.status = element.dataset.status;
  ...
  function activateFilterById(which, id) {
    gridderClose();
    activeTab = which;

    var filterSelectorEl = document.getElementById(id);
  ...
  function updateList() {
    ...
    var activeStatusFilter = document.getElementById('statusSelector');

    var activeThema = activeTab ? activeTab.value : '';
    var activeGebied = activeFilter ? activeFilter.value : '';
    var activeStatus = activeStatusFilter ? activeStatusFilter.value : '';
	  ...
	  sortedElements.forEach(function(element) {
		  ...
		  if (
			  ...
			  (( !activeThema || activeThema == 0 ) || activeThema === elementThema)
				...
				&&
				((!activeStatus || activeStatus == 0) || activeStatus === elementStatus)
		  ){
			  //element.style.display = 'inline-block';
  ...
  function formatEuros(amount, html) {
	  ...
	  if (amount.length > 6) {
	  	var firstPart = amount.substr(0, amount.length - 6);
	  	var secondPart = amount.substr(firstPart.length, 3);
	  	var thirdPart = amount.substr(-3);
	  	
	  	var formatted = firstPart + '.' + secondPart + '.' + thirdPart;
		} else if (amount.length > 3) {
	  	var firstPart = amount.substr(0, amount.length - 3);
	  	var secondPart = amount.substr(firstPart.length, 3);
	  	
	  	var formatted = firstPart + '.' + secondPart;
		} else {
	  	var formatted = amount;
		}

	  return html ? '<span class="eurosign">€ </span><span class="amount">' + formatted + '</span>' : '€ ' +  formatted;
  }
