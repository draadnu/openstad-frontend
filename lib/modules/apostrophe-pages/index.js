  ...
  construct: function (self, options) {
    self.addRankingToIdeas = function (req) {
      ...
      req.data.ideas = req.data.ideas ? req.data.ideas
        .map((idea) => {
          idea.ranking           = idea.extraData.ranking;
          idea.showBudget           = idea.extraData.showBudget;
          return idea;
        }) : [];
    }
    ...
    */
    ...
    self.getUserIdea = require('./getUserIdea.js')(self);
    ...
    self.pageBeforeSend = async (req, callback) => {
      ...
      if (req.query.voteOpinion && req.params.ideaId) {
        req.res.redirect(`/like?ideaId=${req.params.ideaId}&opinion=${req.query.voteOpinion}&redirectUrl=${req.data.currentPath}`)
      }
      ...
      if (ideaId) {
        ...
        if (!req.data.idea) {
          try {
            req.data.idea = await self.getUserIdea(req, ideaId);
          } catch (e) {
            req.data.idea = null;
          }
        }
