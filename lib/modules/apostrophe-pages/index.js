      ...
      self.addRankingToIdeas = function (req) {
        ...
        req.data.ideas =  req.data.ideas ? req.data.ideas
          .sort((ideaA, ideaB) => {
            ...
            if (ideaA.status == 'OPEN') {
              return 1;
            } else if (ideaB.status == 'OPEN') {
              return -1;
            }
            
            if (ideaA.status == 'BUSY' && ideaB.status == 'ACCEPTED') {
              return -1;
            } else if (ideaB.status == 'BUSY' && ideaA.status == 'ACCEPTED') {
              return 1;
            }
            
            if (ideaA.status == 'BUSY' && ideaB.status == 'DENIED') {
              return -1;
            } else if (ideaB.status == 'BUSY' && ideaA.status == 'DENIED') {
              return 1;
            }
            
            if (ideaA.status == 'ACCEPTED' && ideaB.status == 'DENIED') {
              return -1;
            } else if (ideaB.status == 'ACCEPTED' && ideaA.status == 'DENIED') {
              return 1;
            }
            
            if (ideaA.extraData.voteCount && !ideaB.extraData.voteCount) {
              return -1
            } else if (ideaB.extraData.voteCount && !ideaA.extraData.voteCount) {
              return 1;
            }
            
            return (ideaA.extraData.voteCount - ideaB.extraData.voteCount) * -1;
          })
          .map((idea) => {
