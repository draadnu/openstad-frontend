...
module.exports =  function (req, res, next) {
  ...
  if (globalData.siteId) {
    ...
    if (ideas && ideas.length > 0) {
      ...
      // Include arguments for sweelinckplein
      if (globalData.siteId == 43) {
        const uri = `${apiUrl}/api/site/${globalData.siteId}/idea?sort=${sort}&includeVoteCount=1&includeUserVote=1&includeTags=1&includeArguments=1`;
      } else {
        const uri = `${apiUrl}/api/site/${globalData.siteId}/idea?sort=${sort}&includeVoteCount=1&includeUserVote=1&includeTags=1`;
      }
