...
var activeStatusFilter = getCookie('plannenStatusFilter') || 0;
(function() {
  activateIdeaOverviewTab(activeTab)
  activateIdeaOverviewFilter(activeFilter)
  activateStatusFilter(activeStatusFilter)
})();
...
function activateStatusFilter(which) {
  activeFilter = which;
  document.cookie = 'plannenStatusFilter=' + activeFilter;

  var filterSelectorEl = document.getElementById('filterStatusSelector');
  if (which > 0) {
    $(filterSelectorEl).addClass('active');
  } else {
    $(filterSelectorEl).removeClass('active');
  ...
  if (filterSelectorEl) {
    filterSelectorEl.selectedIndex = activeFilter;
    if (filterSelectorEl.selectedIndex === 0) {
      filterSelectorEl.options[0].innerHTML = 'Filter op selectie';
    } else {
      filterSelectorEl.options[0].innerHTML = 'Alle plannen';
    }
  }
...
function updateIdeaOverviewDisplay() {
  ...
  var activeStatusFilter = document.getElementById('filterStatusSelector');

  var activeThema = activeTab ? activeTab.value : '';
  var activeGebied = activeFilter ? activeFilter.value : '';
  var statusFilterValue = activeStatusFilter ? activeStatusFilter.value : '';
  ...
  Array.prototype.forEach.call(elements, function(element) {
    ...
    if (
      ...
      ((!statusFilterValue || statusFilterValue == 0) || element.classList.contains(statusFilterValue))
    ) {
      element.style.display = 'inline-block';
    } else {
