...
const fields = [
      ...
      },
      {
        ...
        label: 'Display status filter?',
        type: 'boolean',
        choices: [
          {
            label: 'Yes',
            value: true,
            showFields: ['statusFilters']
          },
          {
            label: 'No',
            value: false,
      ...
      },
      {
        type: 'checkboxes',
        name: 'statusFilters',
        label: 'Select status(es) to show in filter (select at least one)',
        choices: ideaStates.map((state) => {
          return {
            value: state.value,
            label: state.value,
            showFields: ['filter' + state.value]
          }
        })
      },
      {
        name: 'mapCustomStatuses',
        label: 'Map idea status to a different status for this overview?',
        type: 'boolean',
        choices: [
          {
            label: 'Yes',
            value: true,
            showFields: ['customStatusMapping']
          },
          {
            label: 'No',
            value: false,
      ...
      {
        type: 'checkboxes',
        name: 'customStatusMapping',
        label: 'Select status(es) to map',
        choices: ideaStates.map((state) => {
          return {
            value: state.value,
            label: state.value,
            showFields: ['map' + state.value]
          }
        })
      },
    ].concat(
      ideaStates.map((state) => {
        ...
        ideaStates.map((state) => {
          return {
            type: 'string',
            name: 'filter' +  state.value,
            label: 'Filter label for ' + state.value,
          }
        })
      ).concat(
        ideaStates.map((state) => {
          return {
            type: 'select',
            name: 'map' +  state.value,
            choices: ideaStates.filter((selectState) => {
              return selectState.value != state.value;
            }).map((selectState) => {
              return {
                label: selectState.value,
                value: selectState.value
              }
            }),
            label: 'Map ' + state.value + ' to',
          }
        })
      );
  ...
  construct: function(self, options) {
    options.arrangeFields = (options.arrangeFields || []).concat([
      ...
      },
      {
        ...
        fields: ['displayFilterVoting', 'defaultSorting', 'selectedSorting', 'filterIdeas', 'displayStatusFilter', 'statusFilters'].concat(
          ideaStates.map((state) => {
      ...
      },
      {
        ...
        fields: ['mapCustomStatuses', 'customStatusMapping'].concat(
          ideaStates.map((state) => {
    ...
    const superLoad = self.load;
        ...
        widget.ideas = req.data.ideas.map((idea) => {
          if (self.hasCustomStatusMapping(widget, idea.status)) {
            idea.status = widget['map' + idea.status];
          }
    ...
    //   widget.ideas = enrichIdeas(ideas, googleMapsApiKey);
     ...
     self.hasCustomStatusMapping = function (widget, ideaStatus) {
       return widget.mapCustomStatuses && widget.customStatusMapping.includes(ideaStatus) && widget['map' + ideaStatus];
     }
