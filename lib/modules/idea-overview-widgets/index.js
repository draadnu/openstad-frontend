...
const fields = [
      ...
      {
        type: 'string',
        name: 'labelMinimumVotesReached',
        label: 'Label for: OPEN or CLOSED with minimum votes acquired',
      }
    ].concat(
...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    const superLoad = self.load;
      ...
      widgets = widgets.map(async (widget) => {
        widget.ideas = await self.getIdeas(req);
      ...
      return Promise.resolve(req.data.ideas);
    };
    ...
    //   widget.ideas = enrichIdeas(ideas, googleMapsApiKey);
       ...
       widget.getThemes = function (idea) {
         if (!idea.extraData || !idea.extraData.theme) {
           return [];
         }
    
         return Object.keys(idea.extraData.theme);
       }
       
       widget.displayThemes = function (idea) {
         return widget.getThemes(idea).join(', ');
       }
       
       widget.getHelp = function (idea) {
         if (!idea.extraData || !idea.extraData.hulp) {
           return [];
         }
         
         return Object.keys(idea.extraData.hulp);
       }
       
       widget.displayHelp = function (idea) {
         return widget.getHelp(idea).join(', ');
       }
       
       // Fill filterable themes
       widget.filterableThemes = [];
       
       widget.ideas.forEach((idea) => {
         const themes = widget.getThemes(idea);
         widget.filterableThemes = [...widget.filterableThemes, ...themes];
       })
       
       // Only take unique themes
       widget.filterableThemes = [...new Set(widget.filterableThemes)];
       
       // Fill filterable areas
       widget.filterableAreas = [];
       
       widget.ideas.forEach((idea) => {
         if (idea.extraData && idea.extraData.area) {
           widget.filterableAreas = [...widget.filterableAreas, idea.extraData.area];
         }
       })
       
       // Only take unique themes
       widget.filterableAreas = [...new Set(widget.filterableAreas)];
       
       // Fill help questions
       widget.filterableHelp = [];
       
       widget.ideas.forEach((idea) => {
         const helpItems = widget.getHelp(idea);
         widget.filterableHelp = [...widget.filterableHelp, ...helpItems];
       })
       
       // Only take unique themes
       widget.filterableHelp = [...new Set(widget.filterableHelp)];
