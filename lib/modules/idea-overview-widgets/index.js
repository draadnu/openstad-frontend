...
module.exports = {
  ...
  construct: function(self, options) {
    ...
    //   widget.ideas = enrichIdeas(ideas, googleMapsApiKey);
       ...
       widget.formatImageUrl = function (url, width, height, crop, location, themes, theme, apos) {
         if (url) {
           url = url + '/:/rs=w:' + width + ',h:' + height;
           url = crop ? url + ';cp=w:' + width + ',h:' + height : url;
         } else if (themes && theme) {
           
           var correctTheme = themes.filter(function (item) {
             return item.value == theme;
           })
           
           if (correctTheme) {
             correctTheme = correctTheme[0];
             url = apos.attachments.url(correctTheme.defaultPhoto);
           }
         } else if (location) {
