  ...
  if (ideaForm) {
    ...
    var validator = $(ideaForm).validate({
      ...
      submitHandler: function(form, event) {
        console.log(event);
        $(form).find('button[type="submit"]').val('Verzenden...');
        $(form).find('button[type="submit"]').attr('disabled', true);

        var submitType = $(form).find('button[type="submit"]').data('submit-type');
        var data = $(form);
        console.log(form);
        ...
        $.ajax({
          ...
          success:function(response) {
            ...
            var redirectAfterSave = $(form).find('.form-redirect-save-uri').val();
            redirect = redirect.replace(':id', response.id);
            
            console.log (data, redirect, redirectAfterSave);
            //window.location.replace(redirect);
          },
          error:function(response) {
            // "this" the object you passed
            alert(response.responseJSON.msg);
            $(form).find('button[type="submit"]').val('Opslaan');
