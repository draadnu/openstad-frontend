...
module.exports = {
  ...
  },
  {
    ...
    type: 'string',
    label: 'Title when user is not logged in'
  },
  {
    name: 'notLoggedInContent',
    type: 'string',
...

  {
    ...
    {
     name: 'labelPlaymethod',
     type: 'string',
     label: 'Label for Playmethod',
  },
  {
     name: 'infoPlaymethod',
     type: 'string',
     label: 'Info for Playmethod',
  },
  
    {
      name:       'playmethodOptions',
      type:       'array',
      label:      'Playmethod options',
      titleField: 'value',
      schema:     [
        {
          name:  'value',
    ...
    {
     name: 'labelPlaygroundType',
     type: 'string',
     label: 'Label for PlaygroundType',
  },
  {
     name: 'infoPlaygroundType',
     type: 'string',
     label: 'Info for PlaygroundType',
  },
  
    {
      name:       'playgroundTypeOptions',
      type:       'array',
      label:      'PlaygroundType options',
      titleField: 'value',
      schema:     [
        {
          name:  'value',
  ...
  {
     name: 'displayEnglishCommunication',
     type: 'boolean',
     label: 'Display english communication preference',
     choices: [
       {
         value: true,
         label: "Display"
  ...
  {
     name: 'labelEnglishCommunication',
     type: 'string',
     label: 'Label for english communication preference',
  },
  {
     name: 'infoEnglishCommunication',
     type: 'string',
     label: 'Info for english communication preference',
     textarea: true
  },
  {
  ...
  construct: function(self, options) {
   options.arrangeFields = (options.arrangeFields || []).concat([
     {
       name: 'general',
       label: 'Algemeen',
       fields: ['redirect', 'notLoggedInTitle', 'notLoggedInContent']
     },
    {
       name: 'title',
       label: 'Title',
     ...
     },
     {
       name: 'playmethod',
       label: 'Playmethod',
       fields: ['labelPlaymethod', 'infoPlaymethod', 'playmethodOptions']
     },
     {
       name: 'playgroundType',
       label: 'Playground Type',
       fields: ['labelPlaygroundType', 'infoPlaygroundType', 'playgroundTypeOptions']
     },
     {
       name: 'summary',
       label: 'Summary',
     ...
     },
     {
       name: 'location',
       label: 'Location',
       fields: ['labelLocation', 'infoLocation', 'displayLocation', 'requiredLocation', 'zoomLevelLocation']
     },
     {
         name: 'Estimate',
     ...
     },
     {
       ...
       fields: ['labelEnglishCommunication', 'infoEnglishCommunication', 'displayEnglishCommunication']
     },
     {
       name: 'submitting',
       label: 'Submitting',
			...
			widgets.forEach((widget) => {
				...
				widget.siteConfig = {
					ideas: {
						...
						summaryMinLength: ( siteConfig && siteConfig.ideas && siteConfig.ideas.summaryMinLength ) || 0,
						summaryMaxLength: ( siteConfig && siteConfig.ideas && siteConfig.ideas.summaryMaxLength ) || 140,
						descriptionMinLength: ( siteConfig && siteConfig.ideas && siteConfig.ideas.descriptionMinLength ) || 70,
                ...
                widget.mapConfig = self.getMapConfigBuilder(globalData)
                    .setDefaultSettings({
                        ...
                        mapZoomLevel: (widget.zoomLevelLocation ? widget.zoomLevelLocation : 16),
                        zoomControl: true,
   ...
   self.route('post', 'submit', function(req, res) {
     ...
     if (req.body.action && (req.body.action === 'UPDATE_STATUS' || req.body.action === 'MODBREAK') ) {
       ...
       let summary = req.body.description ? req.body.description.replace(/(\r\n|\n|\r)/gm, "").slice(0, 300) : '';
       
       if (!summary) {
         summary = 'Geen beschrijving opgegeven';
       }
       ...
       if (req.body && req.body.playmethod && req.body.playmethodOther) {
         req.body.playmethod[req.body.playmethod.length - 1] += ' ' + req.body.playmethodOther;
       }

       // get the extra data from the body if set, with pick
       data.extraData = pick(req.body, 'theme', 'area', 'advice', 'phone', 'estimate', 'role', 'englishCommunication', 'playmethod', 'playgroundType');
