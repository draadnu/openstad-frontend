...
module.exports = {
  ...
  },
  {
     ...
     textarea: true,
  },
      {
          ...
          type: 'boolean',
          label: 'Add list items to this field description',
          choices: [
              {
                  value: true,
                  label: "Yes",
                  showFields: ['itemsDescription']
              },
              {
                  value: false,
                  label: "No"
  ...
  construct: function(self, options) {
   ...
   self.pageBeforeSend = function (req) {
     req.data.loginUrl = req.absoluteUrl ? '/oauth/login?returnTo=' + encodeURI(req.absoluteUrl) : '/oauth/login';
   ...
   options.arrangeFields = (options.arrangeFields || []).concat([
     ...
     },
     {
       name: 'description',
       label: 'Description',
       fields: ['labelDescription', 'infoDescription', 'listDescription', 'itemsDescription', 'editorDescription', 'requiredDescription', 'minDescription', 'maxDescription']
     },
     {
       name: 'images',
       label: 'Images Upload',
			...
			widgets.forEach((widget) => {
                ...
                widget.mapConfig = self.getMapConfigBuilder(globalData)
                    .setDefaultSettings({
                        ...
                        mapZoomLevel: (globalData.mapZoomLevel ? globalData.mapZoomLevel : 16),
                        zoomControl: true,
