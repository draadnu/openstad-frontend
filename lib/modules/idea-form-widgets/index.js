  ...
  construct: function(self, options) {
   options.arrangeFields = (options.arrangeFields || []).concat([
     {
       name: 'general',
       label: 'Algemeen',
       fields: ['redirect', 'formType', 'dynamicFormSections', 'notLoggedInTitle', 'notLoggedInContent']
     },
     {
       name: 'title',
       label: 'Title',
   ...
   self.apos.app.use('/image', proxy({
     ...
     onProxyReq : (proxyReq, req, res) => {
       console.log ('/image', imageApiUrl, imageApiToken);
   ...
   self.apos.app.use('/file', proxy({
     ...
     onProxyReq : (proxyReq, req, res) => {
        // add custom header to request
       console.log ('file', imageApiUrl, imageApiToken);
        proxyReq.setHeader('Authorization', `Bearer ${imageApiToken}`);
     }
   }));
