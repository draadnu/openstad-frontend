...
if (window.editorInputElement) {
    // Todo: refactor
  if(mapConfigSetting === 'googlemaps') {
...
$(document).ready(function () {
    ...
    maxSummary = $('#maxSummary').length ? parseInt($('#maxSummary').val()) || 5000 : 5000;
    maxDescription = $('#maxDescription').length ? parseInt($('#maxDescription').val()) || 5000 : 5000;
...
/*  if (ideaFiles) {
  ...
  //      },
       ...
       $.ajax({
        ...
        //  context: document.body,
          ...
          error:function(response) {
              ...
              } else if (response.responseJSON && response.responseJSON.error && response.responseJSON.error) {
                  alert(response.responseJSON.error);
              } else if (response.responseJSON && response.responseJSON.msg) {
                  alert(response.responseJSON.msg)
              } else if (Array.isArray(response.responseJSON)) {
                  alert('Er ging iets fout: ' + response.responseJSON.join(', '));
              } else {
                  alert('Er is iets fout gegaan bij het opslaan, probeer het aub. opnieuw.');
              }
    ...
    if (charsLeftMain) {
      setTimeout(function () {
        var descriptionField  = document.getElementById('js-editor');
        var mainEditor = true;

        if (!descriptionField) {
            descriptionField = document.getElementById('description-textarea');
            mainEditor = false;
        }
        ...
        function updateCharsLeftMain() {
            ...
            var length = maxDescription - stripHTML(value).length;

          /*var textValue = $('#description-textarea').val();
          var length = minLen - textValue.length*/

            if (length > 0) {
                charsLeftMain.style.color = '#9A9A9A';
            } else {
                charsLeftMain.style.color = 'red';
            }
