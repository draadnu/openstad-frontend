...
/*  if (ideaFiles) {
  ...
  if (ideaForm) {
    ...
    $.validator.addMethod("descriptionMinLength", function (value, element, param) {
      ...
      return true;
    }, "Vul hier tussen de " + descriptionMinLength + " en " + descriptionMaxLength + " tekens in, of laat het veld leeg.")
...
window.addEventListener('load', function() {
  ...
  // max 3 field
  if ($('.max-3').length) {
    $('.max-3').on('change', function () {
      var checked = $('.max-3:checked').length;
      
      if (checked == 3) {
        $('.max-3:not(:checked)').attr('disabled', 'disabled');
      } else {
        $('.max-3').removeAttr('disabled');
      }
    }).trigger('change');
    
    $('#other').on('change', function () {
      if ($(this).is(':checked')) {
        $(this).closest('.form-group').find('input[name=playmethodOther]').removeClass('hidden').focus();
      } else {
        $(this).closest('.form-group').find('input[name=playmethodOther]').addClass('hidden').val('');
      }
    }).trigger('change');
  }
